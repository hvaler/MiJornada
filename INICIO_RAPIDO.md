...
### Paso 1: Instalar Paquete Ovillo (2 min)

```powershell
cd C:\MiProyectoExisting
irm <distribution.baseUrl>/install.ps1 | iex
```

El script `arranque.ps1`:
- Descarga la plantilla técnica del ecosistema
...
Al finalizar, crea los archivos de contexto en `_hilo/`.
...
└─────────────┘     └─────────────┘     └─────────────┘     └─────────────┘
   ...
   de carpetas        _hilo/          Folders            completo
```
...
- [ ] Paquete Ovillo instalado (`arranque.ps1`)
...
- **Reglas técnicas del ecosistema**: Ver `Documentos_Base/`
