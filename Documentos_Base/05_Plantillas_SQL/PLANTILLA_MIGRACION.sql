...
-- =============================================
-- Migración: {DescriptiveName}
-- Ticket/Evolutivo: {CodeEvolutivo}
-- Author: {AuthorName}
-- Date: {YYYY-MM-DD}
-- =============================================
-- DESCRIPCIÓN:
--   {DescriptionDetallada}
--
-- CAMBIOS:
--   - {Cambio1}
--   - {Cambio2}
...
PRINT '================================================';
PRINT 'MIGRACIÓN: {DescriptiveName}';
...
BEGIN TRY
    ...
    BEGIN
        RAISERROR('La tabla {Schema}.{Table} no existe. Abortando.', 16, 1);
    END
    ...
    ADD [{NewColumn}] {TipoDato} NULL;
    
    PRINT '  ✓ Columna {NewColumn} agregada';
    
    -- Ejemplo: Agregar constraint
    -- ALTER TABLE [{Schema}].[{Table}]
    -- ADD CONSTRAINT DF_{Table}_{NewColumn} DEFAULT {Valor} FOR [{NewColumn}];
    ...
    -- ALTER TABLE [{Schema}].[{Table}]
    -- ALTER COLUMN [{NewColumn}] {TipoDato} NOT NULL;
    -- PRINT '  ✓ Columna marcada como NOT NULL';
    
    -- Ejemplo: Agregar Foreign Key
    -- ALTER TABLE [{Schema}].[{Table}]
    -- ADD CONSTRAINT FK_{Table}_{TablaRelacionada}
    ...
    FROM [{Schema}].[{Table}] 
    WHERE [{NewColumn}] IS NULL AND Active = 1;
...
BEGIN TRY
    ...
    -- IF EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_{Table}_{TablaRelacionada}')
    -- BEGIN
    --     ALTER TABLE [{Schema}].[{Table}] DROP CONSTRAINT FK_{Table}_{TablaRelacionada};
    ...
    PRINT '  ✓ Columna {NewColumn} eliminada';
