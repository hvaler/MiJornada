# Guía: Instalación y uso de MCPs en Ovillo

> **Versión**: 1.0 (Ovillo v3.8.7)
> **Audiencia**: Desarrolladores
> **Propósito**: Documentar las formas oficiales de instalar y usar Model Context Protocol (MCP) servers en proyectos Ovillo, los patrones recomendados de credenciales, y las mitigaciones de seguridad implementadas en el ecosistema.
...
Ejemplos en uso en la organización:
- **Context7** (`https://mcp.context7.com/mcp`): documentación actualizada de librerías
...
setx JIRA_EMAIL "tu_email@example.com"
...
**Paso 4** — añadir `.claude/settings.local.json` a `.gitignore` (ya viene así por defecto en Ovillo).

### Patrón D — MCP remoto compartido (servidor on-prem la organización)

**Scope**: user · **Mecanismo**: CLI con `--transport http` · **Transporte**: HTTP/SSE · **Auth**: Windows integrada o token corporativo

```bash
claude mcp add miorg-sistema -s user --transport http https://www.example.org/mcp/sistema
```
...
Cuando exista el marketplace propio la organización (`stic-ia-marketplace`), un solo `/plugin install stic-ia/mcps` instalará Context7 + Roslyn + Jira + futuros con configuración pre-empaquetada.
...
## 8. Roadmap MCPs de la organización
...
| Banner la organización | user (corporativo) | 🟡 A diseñar | Auth Azure AD |
...
- `_estado/DECISIONES.md` — ADR sobre selección de MCPs en Ovillo
- `.claude/CLAUDE_BASE.md` §7 — Seguridad (gestión de secretos)
- `.claude/hooks/secret-scanner.js` — Implementación del bloqueo automático

---

*Guía v1.0 - la organización*
