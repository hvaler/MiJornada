...
| Pillar | Description | Tools | Question it answers |
|--------|-------------|----------------|---------------------|
...
public class ScholarshipMetrics
{
    ...
    private static int GetActiveScholarships() => /* query to DB or cache */ 0;
}
```
...
| **Application Insights** | Production | Main platform when cloud.telemetry=app-insights |
...
try
{
    Log.Information("Starting application {ApplicationName}", "MyCompany.MyApp");
...
_logger.LogInformation("Grant {ScholarshipId} created by {User} with amount {Amount}",
...
_logger.LogInformation($"Grant {scholarship.Id} created");  // NO
...
| **Full email** | Personal data | Mask: `j***@example.com` |
