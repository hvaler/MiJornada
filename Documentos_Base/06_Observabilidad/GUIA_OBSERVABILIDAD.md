...
| Pilar | Descripcion | Herramientas de la organización | Pregunta que responde |
|-------|-------------|----------------------|----------------------|
...
var resourceBuilder = ResourceBuilder.CreateDefault()
    .AddService(
        serviceName: "MyCompany.MyApp.Api",
...
builder.Services.AddOpenTelemetry()
    ...
    .WithMetrics(metrics =>
    {
        metrics
            ...
            .AddMeter("MyCompany.MyApp.Business")
            // Exporters
            .AddConsoleExporter()
...
public class ScholarshipService : IScholarshipService
{
    ...
    private static readonly ActivitySource ActivitySource =
        new("MyCompany.MyApp.ScholarshipService", "1.0.0");
    ...
    public async Task<ScholarshipDto> CreateAsync(CreateScholarshipRequest request, CancellationToken ct)
    {
        ...
        activity?.SetTag("scholarship.name", request.Name);
        activity?.SetTag("scholarship.amount", request.Amount);

        try
        {
            var scholarship = Scholarship.Create(request.Name, request.Amount);

            // Sub-actividad para persistencia
            using var dbActivity = ActivitySource.StartActivity("ScholarshipService.Create.Persist");
            await _repository.AddAsync(scholarship, ct);

            activity?.SetTag("scholarship.id", scholarship.Id);
...
public class ScholarshipMetrics
{
    // Un Meter por modulo de negocio
    private static readonly Meter Meter = new("MyCompany.MyApp.Business", "1.0.0");

    // Contadores
    private static readonly Counter<long> ScholarshipsCreated =
        Meter.CreateCounter<long>(
            "myapp.scholarships.creadas",
            unit: "{scholarship}",
            description: "Number total de scholarships creadas");

    private static readonly Counter<long> ApplicationsReceived =
        Meter.CreateCounter<long>(
            "myapp.applications.recibidas",
            unit: "{application}",
            description: "Applications de scholarship recibidas");

    // Histogramas
    private static readonly Histogram<double> TiempoProcesamiento =
        Meter.CreateHistogram<double>(
            "myapp.scholarships.procesamiento.duracion",
            unit: "ms",
            description: "Duration del procesamiento de scholarships");

    // Gauge (valor puntual)
    private static readonly ObservableGauge<int> ActiveScholarships =
        Meter.CreateObservableGauge(
            "myapp.scholarships.activas",
            () => GetActiveScholarships(),
            unit: "{scholarship}",
            description: "Number de scholarships activas actualmente");

    // Metodos publicos para registrar metricas
    public static void RegistrarScholarshipCreated(string tipo)
    {
        ScholarshipsCreated.Add(1, new KeyValuePair<string, object?>("tipo", tipo));
    }

    public static void RegistrarScholarshipApplication(string status)
    {
        ApplicationsReceived.Add(1, new KeyValuePair<string, object?>("status", status));
    }
    ...
    private static int GetActiveScholarships() => /* consulta a BD o cache */ 0;
}
```
...
| **Application Insights** | Produccion | Plataforma principal de la organización (Azure Monitor) |
...
_logger.LogInformation("Scholarship {ScholarshipId} creada por {User} con amount {Amount}",
    scholarship.Id, user, scholarship.Amount);
...
_logger.LogInformation($"Scholarship {scholarship.Id} creada");  // NO
...
public class ScholarshipService : IScholarshipService
{
    ...
    public ScholarshipService(TelemetryClient telemetry)
    {
    ...
    public async Task ProcessLoteAsync(IEnumerable<int> becaIds, CancellationToken ct)
    {
        // Metrica personalizada
        _telemetry.TrackMetric("ScholarshipsProcesadasPorLote", becaIds.Count());

        // Operacion con dependencia
        using var operation = _telemetry.StartOperation<DependencyTelemetry>("ProcessLoteScholarships");
        ...
        try
        {
            ...
            {
                await ProcessScholarshipAsync(id, ct);
            }
...
| Applications de beca por hora | Counter | 25/hora | < 1/hora en periodo abierto |
| Tasa de errores (5xx) | Ratio | 0.1% | > 1% |
| Tiempo de respuesta P95 | Histogram | 250ms | > 1000ms |
| Scholarships activas | Gauge | 45 | < 1 (anomalia) |
...
| **Email completo** | Dato personal | Enmascarar: `j***@example.com` |
...
public class SensitiveDataDestructuringPolicy : IDestructuringPolicy
{
    ...
    {
        if (value is CreateScholarshipRequest request)
        {
