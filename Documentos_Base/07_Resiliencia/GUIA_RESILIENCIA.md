...
### 1.2 Por que importa en la organización
...
var circuitBreakerPipeline = new ResiliencePipelineBuilder<HttpResponseMessage>()
    .AddCircuitBreaker(new CircuitBreakerStrategyOptions<HttpResponseMessage>
    {
        ...
        OnOpened = args =>
        {
            Log.Warning("Circuit Breaker ABIERTO. Duration: {BreakDuration}s. Motivo: {Outcome}",
...
public class OrderProcessorWorker : BackgroundService
{
    ...
    {
        _logger.LogWarning("Shutdown solicitado. Esperando {Count} tasks activas...",
            _activeTasks.Count);

        // Esperar a que terminen las tasks activas (con timeout)
        if (_activeTasks.Any())
        {
            ...
            if (remaining > 0)
            {
                _logger.LogError("Timeout de shutdown. {Count} tasks no completed", remaining);
            }
        }
...
public class CreateScholarshipCommand
{
...
// En el handler
public async Task<ScholarshipDto> Handle(CreateScholarshipCommand request, CancellationToken ct)
{
