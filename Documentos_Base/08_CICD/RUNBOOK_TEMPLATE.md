...
**Causas frecuentes en la organización** (orden de probabilidad):
...
- SDK .NET no instalado en el agente BUILD01 para el `<TargetFramework>` exigido
...
10. Comunicar al equipo + escribir post-mortem en `_hilo/INCIDENTES.md`
...
- ¿Hay dependencia externa caida (Oracle BIPublisher, Azure AD, SQL Server)? → mirar logs de la app, no es problema del pipeline. Avisar a Sistemas si es infra de la organización.
...
|---|---|---|
| Agente offline / capability mal configurada | Sistemas de la organización | `{{CONTACTO_SISTEMAS}}` |
