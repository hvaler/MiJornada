...
> Guía para desarrolladores: cómo auditar tu `azure-pipelines.yml` contra los invariantes G1-G14 Ovillo (G1-G14)
...
2. Lo contrasta contra `.claude/rules/cicd-runtime.md` (los invariantes G1-G14 **R1-R25**) y `.claude/rules/tfs-2020-limitations.md` (tablas A-F de features prohibidas/limitadas en Azure DevOps Server 2020).
...
| 🟡 **SOFT** | Comportamiento inconsistente, deuda conocida o desviación de la convención del ecosistema. Revisar. |
...
```
Audita azure-pipelines.yml con el agent cicd-pipeline-reviewer y dame el reporte R1-R25 con severidad.
```
...
> Requisito: tu proyecto debe tener `.claude/rules/cicd-runtime.md` (G1-G14) — viene con Ovillo ≥ v3.13.0. Si estás en una versión anterior, ejecuta `/actualizar` primero.
...
## Ejemplo real — auditoría de MyCompany.GestionIntercambio (netfx, branch-gated)

Pipeline: .NET Framework 4.8, modelo **branch-gated R24** (develop→Dev/Demo, main→Prod), 4 stages.

**Resultado: APTO — 0 hallazgos 🔴, R1-R25 compliant.** Deuda brownfield conocida y gestionada:
...
| Migrar pipeline TFS Classic (.xaml) | skill `cicd-architect` + `references/classic-migration.md` (manual guiada, ADR-053) |
| Ver estado de builds | `/cicd-status` |

---

*Ovillo v3.16.0 · Las 25 reglas (G1-G14) en `.claude/rules/cicd-runtime.md` · Limitaciones TFS 2020 en `LIMITACIONES_TFS_2020.md`.*
