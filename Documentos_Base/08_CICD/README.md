...
> **Guias tecnicas Ovillo para CI/CD sobre Azure DevOps Server 2020 on-premise** (`devops.example.org`).
> Patron canonico: pool `BUILDERS` server-wide con agentes diferenciados por capability (`Agent.ComputerName=BUILD01` para build, `DeployTarget=<env>` para deploys).
>
> Generado por Ovillo v3.16.0 (ADR-042 + ADR-048/049).
...
| [`AUDITAR_PIPELINE.md`](AUDITAR_PIPELINE.md) | Devs | Como auditar tu `azure-pipelines.yml` contra R1-R25 con el agent `cicd-pipeline-reviewer` (tras editarlo a mano o antes de registrar). Incluye ejemplo real. |
...
## Comandos relacionados (de plantilla Ovillo)
...
- Skill `cicd-architect` — auto-invocacion al editar `azure-pipelines.yml`, propone correcciones segun R1-R25
- Migración TFS Classic (.xaml/.xoml) → YAML: **manual guiada** via `skills/cicd-architect/references/classic-migration.md` (la skill cicd-classic-migrator se retiró en v3.18.0, ADR-053)
- Agent `cicd-pipeline-reviewer` — auditoria YAML contra R1-R25 con severidad por hallazgo
- Hook `pre-cicd-init.js` — gate PreToolUse: verifica version plantilla >= 3.11.0 y conectividad TFS

## Reglas (gating por glob)

- `.claude/rules/cicd-runtime.md` — R1-R25, dispara en `azure-pipelines.yml`
- `.claude/rules/tfs-2020-limitations.md` — tablas A-F (tasks prohibidas, keywords no soportadas, etc.), dispara en YAMLs de pipeline

---

*Documento generado por Ovillo v3.16.0 — Integracion CI/CD (ADR-042 + ADR-048/049)*
