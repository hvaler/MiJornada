...
- [ ] Tests con cobertura adecuada (≥70% line, ≥60% branch — criterio del ecosistema)
- [ ] `appsettings.json` con `Production` correcto en el server destino (NO sobrescribir con tu Development)
- [ ] Secretos en `appsettings.Production.json` del server (NUNCA commiteados — ver `_hilo/DEUDA_TECNICA.md` si aun no estan en Key Vault)
...
```powershell
$app = "<NombreApp>"  # ej: ScholarshipManagement
...
```powershell
$appPool = "<NombreAppPool>"  # ej: ScholarshipManagementAppPool
...
Adicionalmente desde el navegador del dev (fuera del server) abrir `https://<env>.example.org/<TuApp>/` y confirmar 200 con contenido esperado.
...
*Plantilla del runbook deploy manual para Fase 0 — Ovillo v3.11.0 (ADR-042 D11). `/cicd-init` la adapta con datos especificos del proyecto.*
