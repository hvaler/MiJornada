...
> **Origen**: modelo de doble barrera Ovillo local + Pipeline remoto (ver `analisis-integracion-cicd.html` en el portal docs).
...
|---|---|---|---|---|
| **Fase 0** | Cero infraestructura TFS, deploy manual | 0 horas (solo Ovillo local) | `/verify` 7 fases pre-push, recordatorios disciplina | Build remoto, CI verde/rojo, deploy automatico |
| **Fase 1** | Tienes acceso TFS + agentes BUILD01 online | ~2 horas | Build validation branch policy, `dotnet build` + `dotnet test` en cada PR | Deploy automatico, rollback, gates manuales |
...
## Fase 0 — Solo Ovillo local

### Cuando aplica

- Proyecto nuevo o sin pipeline previa.
- Sistemas de la organización aun no ha autorizado BUILDERS al proyecto.
...
2. **Recordatorios pre-push** en `_hilo/CLAUDE.md` (sección "Pre-commit"): obliga a ejecutar `/verify` antes de cada `git push`.
3. **Entrada en `_hilo/ESTADO_PROYECTO.json.infraestructura.cicd[]`** con `{ fase: 0, ... }`.
...
Cuando Sistemas autorize BUILDERS al proyecto + agente BUILD01 disponible:
...
- Agente BUILD01 online (Sistemas confirma).
...
5. **Entrada en `_hilo/ESTADO_PROYECTO.json.infraestructura.cicd[]`** con `{ fase: 1, pipelineId, url, stages: ['Build'] }`.
...
Cuando Sistemas instale agentes `<SERVER>_DEPLOY` con capability `DeployTarget=<env>-<app|svc>` (R1/R25: un agente por entorno×tipo — Aplicaciones y Servicios; ver `PERMISOS_CICD.md` §3) en cada server destino:
...
- ❌ NUNCA `DownloadBuildArtifacts@0` / `DownloadPipelineArtifact@2` (bug SSL Node.js contra `devops.example.org`) → usar REST API + PowerShell
...
|---|---|---|---|
| `_hilo/ESTADO_PROYECTO.json` configurado | ✅ | ✅ | ✅ |
...
- ❌ **Tratar Fase 0 como "no se hace CI/CD"**: Fase 0 SI es CI/CD — la primera barrera (Ovillo local con `/verify` 7 fases) es lo que evita bugs reales en deploys manuales.
- ❌ **Editar `azure-pipelines.yml` ignorando R1-R25**: el agent `cicd-pipeline-reviewer` te avisara, pero la disciplina humana es mas barata.
...
## Actualizar el pipeline tras nuevas versiones de Ovillo

Hay **dos capas** y actualizar una NO actualiza la otra:

| Capa | Que es | Como se actualiza |
|---|---|---|
| **Plantilla Ovillo** (`.claude/`) | Las *herramientas*: comandos, reglas, skill `cicd-architect`, agent `cicd-pipeline-reviewer`, hooks, templates | `irm <distribution.baseUrl>/install.ps1 \| iex` |
...
| Tipo de mejora en Ovillo | ¿Tocar el YAML? |
|---|---|
...
*Documento generado por Ovillo v3.16.0 — Integracion CI/CD (ADR-042 D11 + ADR-043 + ADR-048/049). Seccion "Actualizar el pipeline" anadida 2026-06-02.*
